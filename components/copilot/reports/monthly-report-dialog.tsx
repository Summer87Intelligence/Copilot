...
export type MonthlyReportDialogProps = {
  open: boolean;
  onClose: () => void;
  title: string;
  description: string;
  buildUrl: (year: number, month: number, currency: "UYU" | "USD") => string;
  defaultFilename: (year: number, month: number, currency: "UYU" | "USD") => string;
};
...
export function MonthlyReportDialog({
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError(null);
    try {
      const url = buildUrl(year, month, currency);
      ...
      a.href = objectUrl;
      a.download = match?.[1] ?? defaultFilename(year, month, currency);
  ...
  }, [year, month, currency, buildUrl, defaultFilename, onClose]);
    ...
    >
      <div className="w-full max-w-sm rounded-2xl border border-[var(--copilot-border)] bg-[var(--copilot-card)] shadow-xl">
        <div className="flex items-start justify-between gap-3 border-b border-[var(--copilot-border)] px-5 py-4">
          <div>
            <h2
              id="monthly-report-dialog-title"
