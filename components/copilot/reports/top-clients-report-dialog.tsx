...
type SortBy = "net_sales" | "debt" | "overdue";
...
export function TopClientsReportDialog({ open, onClose }: Props) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/copilot/reports/top-clients.pdf?year=${year}&month=${month}&currency=${currency}&sort=${sortBy}`;
      ...
      a.href = objectUrl;
      a.download = match?.[1] ?? `clientes-principales-${year}-${mm}-${currency}-${sortBy}.pdf`;
  ...
  }, [year, month, currency, sortBy, onClose]);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="top-clients-report-title"
      onMouseDown={(e) => {
...
export function TopClientsReportTrigger({
  className = "",
  label = "Generar PDF",
}: {
  className?: string;
  ...
  return (
    <>
      ...
      <TopClientsReportDialog open={open} onClose={() => setOpen(false)} />
    </>
  );
}
