...
function evidenceBlock(insight: CopilotRealInsight): string {
  switch (insight.type) {
    case "deuda_vencida":
      return `Monto atrasado: $ ${insight.evidence.amount.toLocaleString("es-AR", { maximumFractionDigits: 0 })} · Hasta ${insight.evidence.days_overdue} días de atraso desde el vencimiento más lejano`;
    case "concentracion_deuda":
      return `Atrasado total: $ ${insight.evidence.total_overdue.toLocaleString("es-AR", { maximumFractionDigits: 0 })} · Top 2: $ ${insight.evidence.top2_amount.toLocaleString("es-AR", { maximumFractionDigits: 0 })} (${(insight.evidence.top2_share_pct * 100).toLocaleString("es-AR", { maximumFractionDigits: 1 })}%)`;
...
const typeLabel: Record<CopilotRealInsight["type"], string> = {
  ...
  obl_fiscal_vencida: "Obligación fiscal atrasada",
