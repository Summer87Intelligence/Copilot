...
export function CurrencyExecutiveCard({ block }: { block: CurrencyExecutiveBlock }) {
  ...
  const showCollectionNote = shouldShowCollectionExceedsBillingNote(
    block.billedPeriod?.amount,
    block.collectedPeriod?.amount
  );
  ...
  return (
    <div className="rounded-2xl border border-[var(--copilot-border)] bg-[var(--copilot-card-bg)] p-5 shadow-sm">
      ...
      <div className="divide-y divide-[var(--copilot-border)]/50">
        <MetricLine
          label={CURRENCY_METRIC_LABELS.billed}
          helper={CURRENCY_METRIC_HELPERS.billed}
          amount={block.billedPeriod}
          tone="neutral"
        />
        <MetricLine
          label={CURRENCY_METRIC_LABELS.collected}
          helper={CURRENCY_METRIC_HELPERS.collected}
          amount={block.collectedPeriod}
          tone="positive"
          suffix={ratePct}
        />
        <MetricLine
          label={CURRENCY_METRIC_LABELS.pending}
          helper={CURRENCY_METRIC_HELPERS.pending}
        ...
        <MetricLine
          ...
          helper={CURRENCY_METRIC_HELPERS.overdue30}
        ...
        <div className="mt-3 border-t border-dashed border-[var(--copilot-border)] pt-2">
          ...
          {showAging ? (
            <div className="mt-2 space-y-0 rounded-lg bg-[rgba(44,40,37,0.03)] px-2 py-1">
              <MetricLine
                label={HOY_COPY.agingCurrentLabel}
                amount={block.expectedIncomeCurrent}
                tone="positive"
                empty="—"
              />
              <MetricLine
                label={HOY_COPY.agingOverdueLabel}
