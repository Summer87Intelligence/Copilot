/** Fix mutation handlers that should use requireCopilotModuleWriteAccess. */
import fs from "node:fs";
import path from "node:path";

const ROOT = process.cwd();
const DIRS = [
  "app/api/copilot/data",
  "app/api/copilot/treasury",
  "app/api/copilot/collection-actions",
  "app/api/copilot/actions",
  "app/api/copilot/clients",
];
...
const MUTATION = /export async function (POST|PUT|PATCH|DELETE)\b/g;
...
function patchFile(file) {
  ...
  if (
    ...
    !out.includes("requireCopilotModuleWriteAccess,") &&
    out.includes('from "@/lib/auth/copilot-module-api-auth"') &&
