 ...
 * - Solo comprobantes dentro del período operativo 2026 (jan 2026+).
 *
 * Protecciones:
...
const PIPELINE = ZETA_PIPELINE_NAMES.VENDOR_PAYMENTS;
...
type WorkspaceSummary = {
  ...
  rows_updated: number;
  rows_skipped: number;
  invalid_fecha_count: number;
  negative_amount_count: number;
  pre_operational_date_count: number;
  errors: number;
};
...
export async function GET(request: NextRequest) {
  ...
  let totalUpdated = 0;
  let totalSkipped = 0;
  let totalInvalidFecha = 0;
  let totalNegativeAmount = 0;
  let totalPreOperational = 0;
  let totalFailed = 0;
  ...
  while (true) {
    ...
    for (const workspaceId of page.ids) {
      ...
      let wsUpdated = 0;
      let wsSkipped = 0;
      let wsInvalidFecha = 0;
      let wsNegativeAmount = 0;
      let wsPreOperational = 0;
      let wsErrors = 0;
      let monthsSynced = 0;
      ...
      for (let mi = 0; mi < syncMonths.length; mi++) {
        ...
        try {
          ...
          wsInvalidFecha += outcome.invalid_date_rows ?? 0;
          wsNegativeAmount += outcome.negative_amount_rows ?? 0;
          wsPreOperational += outcome.pre_operational_rows ?? 0;
      ...
      totalInvalidFecha += wsInvalidFecha;
      totalNegativeAmount += wsNegativeAmount;
      totalPreOperational += wsPreOperational;
      workspaceSummaries.push({
        ...
        invalid_fecha_count: wsInvalidFecha,
        negative_amount_count: wsNegativeAmount,
        pre_operational_date_count: wsPreOperational,
        errors: wsErrors,
      });
  ...
  return NextResponse.json({
    ...
    persisted_count: totalUpdated,
    invalid_fecha_count: totalInvalidFecha,
    negative_amount_count: totalNegativeAmount,
    pre_operational_date_count: totalPreOperational,
    total_failed: totalFailed,
    duration_ms: duration,
