...
import { addSuggestionNote } from "@/lib/bank/review/bank-review-actions.server";
...
const noteBodySchema = z.object({
  ...
  clientToken: z.string().trim().min(1).max(64).optional(),
});
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = await parseAndValidateJsonBody(request, noteBodySchema);
  ...
  const result = await addSuggestionNote(
    supabase,
    tenantCompanyId,
    suggestionId,
    appUser.id,
    parsed.data.note,
    parsed.data.clientToken ?? null
  );
