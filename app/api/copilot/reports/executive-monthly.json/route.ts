...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!auth.ok) {
      log.warn("copilot_auth_failed", { phase: "executive_monthly_report_json" });
    ...
    log.info("executive_monthly_report_json_ready", {
    ...
    log.error("executive_monthly_report_json_failed", {
      error: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
      { ok: false, error: "No se pudo cargar el reporte ejecutivo." },
      { status: 500 }
    );
  }
}
