...
import type { ProtoInvoicePatch } from "@/lib/copilot-proto-crud-types";
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const raw = body as { id?: string } & ProtoInvoicePatch;
      ...
      return NextResponse.json(
        {
          ok: false as const,
          code: "VALIDATION" as const,
          message: "Falta el identificador de la factura a editar.",
        },
        { status: 400 }
      );
    }
    const result = await protoUpdateInvoice(supabase, id, patch);
