...
import type { ProtoCompanyPatch } from "@/lib/copilot-proto-crud-types";
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const raw = body as { id?: string } & ProtoCompanyPatch;
      ...
      return NextResponse.json(
        {
          ok: false as const,
          code: "VALIDATION" as const,
          message: "Falta el identificador de la empresa a editar.",
        },
        { status: 400 }
      );
    }
    const result = await protoUpdateCompany(supabase, id, patch);
