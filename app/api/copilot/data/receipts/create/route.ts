...
import { protoReceiptCreateBodySchema } from "@/lib/api/schemas/copilot-api-bodies";
import { requireCopilotTenantContext } from "@/lib/copilot-api-auth";
import { MSG_DB_USER } from "@/lib/copilot-data-integrity";
import { nextResponseFromProtoCrud } from "@/lib/copilot-proto-crud-http";
import { protoCreateReceipt } from "@/lib/copilot-proto-crud-service";

export async function POST(request: NextRequest) {
  try {
    const pv = await parseAndValidateJsonBody(
      request,
      protoReceiptCreateBodySchema
    );
    ...
    const result = await protoCreateReceipt(auth.ctx.supabase, body);
