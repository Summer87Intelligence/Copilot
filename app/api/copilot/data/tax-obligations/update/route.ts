...
import type { ProtoTaxObligationPatch } from "@/lib/copilot-proto-crud-types";
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const raw = body as { id?: string } & ProtoTaxObligationPatch;
      ...
      return NextResponse.json(
        {
          ok: false as const,
          code: "VALIDATION" as const,
          message: "Falta el identificador de la obligación fiscal a editar.",
        },
        { status: 400 }
      );
    }
    const result = await protoUpdateTaxObligation(supabase, id, patch);
