...
import { getOrRefreshReconciliationSnapshot } from "@/lib/operacional/oic-snapshot-service";
import { isOicEnabled } from "@/lib/operacional/oic-feature-flag";
import type { OicApiResponse, OicReconciliationSummary } from "@/lib/operacional/types";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const snapshot = await getOrRefreshReconciliationSnapshot(supabase, workspaceCompanyId);
    const data = snapshot.payload as unknown as OicReconciliationSummary;

    return NextResponse.json({
      ok: true,
      data,
      computedAt: snapshot.updated_at,
    } satisfies OicApiResponse<OicReconciliationSummary>);
  } catch (e) {
