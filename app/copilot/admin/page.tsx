    ...
    <ModalOverlay onClose={onClose}>
      <div className="px-5 py-5">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          <span className="flex-1 font-mono text-xl font-bold tracking-widest text-amber-900">
            {shown ? pin : "••••••"}
          </span>
          <button
            type="button"
          ...
          <button
            ...
            title="Copiar"
          >
            {copied ? <Check className="h-4 w-4 text-emerald-600" /> : <RefreshCw className="h-4 w-4" />}
          </button>
        </div>
...
export default function AdminPanelPage() {
  ...
  async function handleResetPin(user: AdminUser) {
    if (!confirm(`¿Resetear el PIN de ${user.full_name}? Su sesión actual se invalidará.`)) return;
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.ok) {
        alert(data.message ?? "Error al resetear PIN.");
        return;
      }
    ...
    <main className={copilotPageMainClass}>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <CopilotKpiCard
          ...
          hint="Con acceso al workspace"
        />
        <CopilotKpiCard
          label="Solo lectura"
          value={loading ? "—" : String(summary.readOnly)}
          hint="usuario / demo_readonly"
        />
        <CopilotKpiCard
          label="Superadmins"
      ...
      {/* Error */}
      {error && (
        <CopilotCard className="flex items-center gap-2 border-rose-200 bg-rose-50">
      ...
      <CopilotCard className="overflow-hidden p-0">
        ...
        {!loading && users.length === 0 && !error && (
          <div className="py-12 text-center">
            ...
            <p className="text-sm text-[var(--copilot-ink-muted)]">No hay usuarios en este workspace.</p>
          </div>
        )}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            type="button"
                            title="Resetear PIN"
                            onClick={() => void handleResetPin(user)}
                            disabled={isLoadingPin}
                            className="rounded-lg p-1.5 text-[var(--copilot-ink-muted)] hover:bg-amber-50 hover:text-amber-700"
                          >
                            {isLoadingPin ? (
                              <Loader2 className="h-3.5 w-3.5 animate-spin" />
                            ) : (
      ...
      {/* Security notice */}
      <CopilotCard className="flex items-start gap-2.5 border-blue-100 bg-blue-50/60">
        ...
        <div className="text-xs text-blue-800">
          ...
          <ul className="mt-1 list-inside list-disc space-y-0.5 text-blue-700">
            <li>No compartir el usuario superadmin entre personas.</li>
            <li>Usá <em>demo_readonly</em> para mostrar el sistema sin riesgo.</li>
            <li>Resetear el PIN invalida sesiones activas de ese usuario.</li>
            <li>Mantener siempre al menos un superadmin activo.</li>
          </ul>
        </div>
      </CopilotCard>
