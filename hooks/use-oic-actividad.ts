...
import type { OicPipelineActivityData } from "@/lib/operacional/types";

type State = {
  data: OicPipelineActivityData | null;
  loading: boolean;
  error: string | null;
...
type Action =
  | { type: "request" }
  | { type: "success"; data: OicPipelineActivityData; fetchedAt: string }
...
export type UseOicActividadResult = {
  data: OicPipelineActivityData | null;
  loading: boolean;
  error: string | null;
  errorCode: string | null;
...
export function useOicActividad(enabled = true): UseOicActividadResult {
  ...
  useEffect(() => {
    ...
    fetch("/api/operacional/actividad", {
      method: "GET",
