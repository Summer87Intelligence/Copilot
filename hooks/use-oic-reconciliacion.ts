...
type State = {
  data: OicReconciliationSummary | null;
  loading: boolean;
  error: string | null;
...
type Action =
  | { type: "request" }
  | { type: "success"; data: OicReconciliationSummary; fetchedAt: string }
...
export type UseOicReconciliacionResult = {
  data: OicReconciliationSummary | null;
  loading: boolean;
  error: string | null;
  errorCode: string | null;
...
export function useOicReconciliacion(enabled = true): UseOicReconciliacionResult {
  ...
  useEffect(() => {
    ...
    fetch("/api/operacional/reconciliacion", {
      method: "GET",
