...
import type { OicSnapshotSummary } from "@/lib/operacional/types";

type State = {
  data: OicSnapshotSummary[] | null;
  loading: boolean;
  error: string | null;
...
type Action =
  | { type: "request" }
  | { type: "success"; data: OicSnapshotSummary[]; fetchedAt: string }
...
export type UseOicSnapshotsResult = {
  data: OicSnapshotSummary[] | null;
  loading: boolean;
  error: string | null;
  errorCode: string | null;
...
export function useOicSnapshots(limit = 50, enabled = true): UseOicSnapshotsResult {
  ...
  useEffect(() => {
    ...
    fetch(`/api/operacional/snapshots?limit=${limit}`, {
      method: "GET",
    ...
    return () => ac.abort();
  }, [enabled, limit, state.refreshTick]);
