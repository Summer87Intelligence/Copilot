-- BANK-CANONICAL-CONFIRM-CONTRACT-CORRECTION-001
--
-- Corrige `confirm_bank_reconciliation_v1` (v1, migración 20260719120200). NO
-- toca ningún CHECK constraint ni ninguna tabla. Dos correcciones, ambas
-- puramente dentro del cuerpo de la función:
--
-- 1) BUG MUERTO: la v1 comparaba `v_mov.status IN ('ignored','reversed')`,
--    pero `bank_movements.status` NUNCA admitió 'reversed' (CHECK real:
--    pending|suggested|matched|ignored|needs_review — ver migración
--    20260709120938 y `BankMovementStatus` en TypeScript, ambos coinciden).
--    Ningún writer escribe jamás 'reversed' en esa columna (ni esta RPC ni
--    `reverse_bank_reconciliation_v1`, que solo archiva el LINK — nunca toca
--    `bank_movements`). La comparación era inalcanzable, no un bug que rompa
--    producción: cambio de CERO comportamiento real, solo retira una rama
--    muerta que sugería (incorrectamente) que el esquema soporta ese valor.
--    NO se amplía el CHECK de `bank_movements.status` — la auditoría concluyó
--    que la inconsistencia es de LÓGICA de la RPC, no de esquema (ver
--    docs/architecture/bank-reconciliation-canonical-engine.md, sección
--    "Auditoría exacta del estado reversed").
--
-- 2) GAP REAL: la v1 no validaba `suggestion_scope` al confirmar por
--    `p_suggestion_id` — una sugerencia `historical_review` o `matched_audit`
--    podía confirmarse igual que una `operational`. Se agrega
--    `AND suggestion_scope = 'operational'` a esa validación: la RPC canónica
--    de confirmación diaria SOLO puede confirmar sugerencias operativas.
--
-- `reverse_bank_reconciliation_v1` no requiere cambios (auditada de nuevo en
-- esta fase: idempotente, opera sobre `link_id` con lock, no depende del
-- valor cuestionado).
--
-- SEGURIDAD: mismo contrato que v1 — SECURITY INVOKER, search_path fijo, sin
-- SQL dinámico, EXECUTE solo para `service_role`. ADITIVA (CREATE OR REPLACE,
-- misma firma). NO APLICAR sin autorización.
...
BEGIN
  ...
  -- CORRECCIÓN 1: 'reversed' NUNCA es un valor válido de bank_movements.status
  -- (CHECK real: pending|suggested|matched|ignored|needs_review). La v1 lo
  -- comparaba igual (rama inalcanzable) — se retira aquí, cero cambio de
  -- comportamiento real.
  ...
  -- confirmable, y CORRECCIÓN 2: ser 'operational' — nunca se confirma una
  -- sugerencia histórica ni matched_audit por esta vía canónica diaria.
  IF p_suggestion_id IS NOT NULL THEN
...
COMMENT ON FUNCTION public.confirm_bank_reconciliation_v1(uuid,uuid,uuid,uuid,jsonb,numeric,uuid) IS
  'Confirma conciliación en UNA transacción: crea link canónico + allocations (agregadas por factura), valida sumas (movimiento/recibo/factura) con locks, permite saldo sin aplicar, idempotente con detección de conflicto. Solo confirma sugerencias suggestion_scope=operational. Solo service_role. FASE BANK-CANONICAL-CONFIRM-CONTRACT-CORRECTION-001.';

-- Permisos: sin cambios (ya eran service_role-only desde la v1), se re-declaran
-- por completitud/idempotencia de la migración.
...
-- ROLLBACK CONCEPTUAL (no ejecutar aquí): restaurar el cuerpo de la v1
-- (supabase/migrations/20260719120200_bank_reconciliation_confirm_rpc.sql)
-- vía otro CREATE OR REPLACE FUNCTION con la misma firma.
